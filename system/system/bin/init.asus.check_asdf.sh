...
action_log='/proc/asusevtlog'		#set /dev/null to turn off log
...
android_reboot_prop='vendor.asus.android_reboot'
...
rebootReason=`getprop sys.boot.reason.last`
imageVersion=`getprp ro.vendor.build.asus.version`

echo "reboot reason:"$rebootReason > /proc/asusevtlog
echo $imageVersion > /proc/asusevtlog
...
if [ ".$act_code" != "." ]; then
	...
	if [ ".$android_reboot" == "." ]; then
		...
		chmod 777 /asdf/lastlogcat.txt
		chmod 777 /asdf/lastlogcat.txt.1
#for atcmd checksum
...
#for atcmd checksum		
		...
		if [ $fcount -gt 0 ]; then
			...
			if [ -e $log_dir.1.tar.gz ]; then
				...
				i=$(($max_log-1))
				while [ $i -gt 0 ]; do
					if [ -e $log_old.$i.tar.gz ]; then
						echo "$log_head: rotate log_dir.$i to log_dir.$(($i+1))" > $action_log
						mv $log_old.$i.tar.gz $log_dir.$(($i+1)).tar.gz
					fi
					i=$(($i-1))
				done
				rm -r $log_root.old
			...
			cp /asdf/qsee_log.txt  $log_dir.1/
			cp /asdf/tz_log.txt  $log_dir.1/
			cp /asdf/last_kmsg $log_dir.1/
			cp /asdf/last_logcat  $log_dir.1/
			cp /asdf/logcat-crash.txt $log_dir.1/
			...
			chmod 777 $log_root
			tar zcvf $log_dir.1.tar.gz $log_dir.1 	&& rm -R $log_dir.1
			chmod -R 666 $log_dir.1.tar.gz
