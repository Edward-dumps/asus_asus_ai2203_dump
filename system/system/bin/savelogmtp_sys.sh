...
############################################################################################	
	...
	SAVE_LOG_PATH=`getprop vendor.asus.savelogmtp.folder`
...
######################################################################################
	...
	cat /sys/devices/platform/11d04000.i2c/i2c-7/7-0053/rt9490-chg.1.auto/dump_all_registers > $SAVE_LOG_PATH/charger_debug/rt9490_reg.txt
	cat /sys/devices/platform/11d04000.i2c/i2c-7/7-0065/dump_all_registers > $SAVE_LOG_PATH/charger_debug/nu2205_reg.txt
	cat /sys/class/power_supply/*/uevent > $SAVE_LOG_PATH/charger_debug/psy_event.txt
...
############################################################################################
	...
	#add to stop and then capture modem log problem
############################################################################################
	# copy modem mini ramdumps
	cp -r /data/ramdump/ $SAVE_LOG_PATH
	rm -r /data/ramdump/
	echo "cp -r /data/ramdump $SAVE_LOG_PATH"
...
############################################################################################
	timeout 5 top  > $SAVE_LOG_PATH/top.txt
############################################################################################
...
#        dumpstate -q -d -z  -o $SAVE_LOG_PATH/bugreport
	...
	dmabuf_dump > $SAVE_LOG_PATH/dmabuf_dump.txt 
...
cp -r /metadata/ota $SAVE_LOG_PATH
	...
	dd if=/dev/block/by-name/expdb  of=$SAVE_LOG_PATH/lk.txt bs=1 count=2093040 skip=0x07E00000
	
	cp -r /data/debuglogger/ $SAVE_LOG_PATH
	cp -r /data/aee_exp  $SAVE_LOG_PATH
	procrank > $SAVE_LOG_PATH/procrank.txt
	rm -rf /data/debuglogger/*
	rm /asdf/vendor_aee_exp.tar.gz
	#procrank > $SAVE_LOG_PATH/procrank.txt
